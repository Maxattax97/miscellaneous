...
hdd_assert_ro() {
    if mountpoint /mnt/Seagate2TB; then
        ...
        if [ "$status" = "" ]; then
            ...
            echo "$status"
            read -r -p "Remount RO? (Y/N) " choice
            case "$choice" in
                y | Y)
                    hdd_mount_ro
                    ...
                    ;;
                n | N)
                    read -r -p "You're about to break your file system. Are you sure? Type YES to confirm: " wellfuck
                    case "$wellfuck" in
                        YES)
                            echo "Good luck!"
                            return 0
...
vfio_winvm() {
    if hdd_assert_ro; then echo "HDD available!"; else
        echo "HDD unavailable, exiting..."
        return 1
    fi
    vfio_enable_hugepages

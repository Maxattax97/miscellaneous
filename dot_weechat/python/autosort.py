...
def parse_int(arg, arg_name="argument"):
    ...
    try:
        ...
        raise HumanReadableError(
            'Invalid {0}: expected integer, got "{1}".'.format(arg_name, arg)
        )
...
def decode_rules(blob):
    ...
    if not isinstance(parsed, list):
        log(
            "Malformed rules, expected a JSON encoded list of strings, but got a {0}. No rules have been loaded. Please fix the setting manually.".format(
                type(parsed)
            )
        )
        return []
    ...
    for i, entry in enumerate(parsed):
        if not isinstance(entry, (str, unicode)):
            log(
                "Rule #{0} is not a string but a {1}. No rules have been loaded. Please fix the setting manually.".format(
                    i, type(entry)
                )
            )
            return []
...
def decode_helpers(blob):
    ...
    if not isinstance(parsed, dict):
        log(
            "Malformed helpers, expected a JSON encoded dictionary but got a {0}. No helpers have been loaded. Please fix the setting manually.".format(
                type(parsed)
            )
        )
        return {}
    ...
    for key, value in parsed.items():
        if not isinstance(value, (str, unicode)):
            log(
                'Helper "{0}" is not a string but a {1}. No helpers have been loaded. Please fix setting manually.'.format(
                    key, type(value)
                )
            )
            return {}
...
class Config:
    ...
    def save_helpers(self, run_callback=True):
        ...
        weechat.config_option_set(
            self.__helpers, json.dumps(self.helpers), run_callback
        )
...
def split_args(args, expected, optional=0):
    ...
    if len(split) < expected:
        raise HumanReadableError(
            "Expected at least {0} arguments, got {1}.".format(expected, len(split))
        )
...
def do_sort(verbose=False):
    ...
    buffers = sort_buffers(
        hdata, buffers, config.rules, config.helpers, config.case_sensitive
    )
...
def on_signal(data, signal, signal_data):
    ...
    if sort_limit_timer is not None:
        if sort_queued:
            debug(
                "Signal {0} ignored, sort limit timeout is active and sort is already queued.".format(
                    signal
                )
            )
        else:
            debug(
                "Signal {0} received but sort limit timeout is active, sort is now queued.".format(
                    signal
                )
            )
        sort_queued = True
...
def on_signal_delay_timeout(pointer, remaining_calls):
    ...
    if config.sort_limit > 0:
        ...
        sort_limit_timer = weechat.hook_timer(
            config.sort_limit, 0, 1, "on_sort_limit_timeout", ""
        )
...
def add_completions(completion, words):
    for word in words:
        weechat.hook_completion_list_add(
            completion, word, 0, weechat.WEECHAT_LIST_POS_END
        )
...
def autosort_complete_rules(words, completion):
    if len(words) == 0:
        add_completions(
            completion, ["add", "delete", "insert", "list", "move", "swap", "update"]
        )
...
if weechat.register(
    ...
    weechat.hook_command(
        "autosort",
        command_description.format(**colors),
        "",
        "",
        command_completion,
        "on_autosort_command",

...
def color_nick(nick):
    ...
    if nick[0] in modes:
        ...
        mode_color = wcolor(
            config_string("weechat.color.nicklist_prefix%d" % (modes.find(mode) + 1))
        )
    else:
        mode = mode_color = ""
    # nick color
    ...
    if nick:
        ...
        if not nick_color:
            ...
            nick_color = wcolor(
                config_string("weechat.color.chat_nick_color%02d" % idx)
            )
    return "".join(
        (prefix_c, prefix, mode_color, mode, nick_color, nick, suffix_c, suffix)
    )
...
def get_config_boolean(config):
    ...
    try:
        ...
        error(
            "Error while fetching config '%s'. Using default value '%s'."
            % (config, default)
        )
...
def get_file_by_name(buffer_name):
    ...
    for config in config_masks:
        ...
        if "$name" in mask:
            ...
            if (
                "." in buffer_name and "#" not in buffer_name[: buffer_name.find(".")]
            ):  # the dot isn't part of the channel name
...
def show_matching_lines():
    ...
    if search_in_buffers:
        ...
        for buffer in search_in_buffers:
            ...
            matched_lines[buffer_name] = grep_buffer(
                buffer,
                head,
                tail,
                after_context,
    ...
    if search_in_files:
        ...
        if not background:
            ...
            if hook_file_grep:
                buffer_create(
                    "Searching for '%s' in %s worth of data..."
                    % (pattern_tmpl, human_readable_size(size))
                )
    else:
        buffer_update()
...
def get_grep_file_status():
    ...
    return (
        'Searching in %s, running for %.4f seconds. Interrupt it with "/grep stop" or "stop"'
...
def buffer_update():
                    ...
                    for line in lines:
                        ...
                        if line == linesList._sep:
                            ...
                            if "\x00" in line:
                                # log was corrupted
                                error(
                                    "Found garbage in log '%s', maybe it's corrupted"
                                    % log
                                )
    ...
    title = (
        ...
        % (
            ...
            matched_lines,
            color_reset,
            matched_lines.get_matches_count(),
            note,
            color_title,
            pattern_tmpl,
            color_reset,
    ...
    def tmplReplacer(match):
        ...
        try:
            ...
            if callable(template):
                ...
                if not r:
                    error(
                        "Template %s returned empty string "
...
def completion_grep_args(data, completion_item, buffer, completion):
    for arg in (
        ...
        weechat.hook_completion_list_add(
            completion, "--" + arg, 0, weechat.WEECHAT_LIST_POS_SORT
        )
    for tmpl in templates:
        weechat.hook_completion_list_add(
            completion, "%{" + tmpl, 0, weechat.WEECHAT_LIST_POS_SORT
        )
    return WEECHAT_RC_OK
...
""",
    ...
    weechat.hook_command(
        ...
        " <filter>: Only show logs that match <filter>. Use '*' and '?' as wildcards.",
        "--size",
        "cmd_logs",
        "",
    )
    ...
    weechat.hook_completion(
        "grep_log_files", "list of log files", "completion_log_files", ""
    )
    weechat.hook_completion(
        "grep_arguments", "list of arguments", "completion_grep_args", ""
    )

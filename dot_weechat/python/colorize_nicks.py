            ...
            try:
                ...
                if w.config_boolean(colorize_config_option["greedy_matching"]):
                    ...
                    for word in line.split():
                        ...
                        if w.config_boolean(
                            colorize_config_option["ignore_nicks_in_urls"]
                        ) and word.startswith(("http://", "https://")):
                            continue
                        ...
                        if nick in word:
                            ...
                            if len(biggest_nick) > 0 and biggest_nick in word:
                                ...
                                len(word) < len(biggest_nick) or len(biggest_nick) == 0
                            ):
                                new_word = word.replace(
                                    nick, "%s%s%s" % (nick_color, nick, reset)
                                )
...
def update_blacklist(*args):
    ...
    ignore_channels = w.config_string(
        colorize_config_option["blacklist_channels"]
    ).split(",")

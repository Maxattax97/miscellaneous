...
settings = {
    "interval": (
        "0",
        "How often (in seconds) to force an update of all items. 0 means deactivated",
    ),
}
...
def toggle_refresh_cb(pointer, name, value):
    option_name = name[
        len("plugins.var.python." + SCRIPT_NAME + ".") :
    ]  # get optionname
...
def substitute_colors(text, window):
    if int(version) >= 0x00040200:
        ...
        return weechat.string_eval_expression(
            text, {"window": window, "buffer": bufpointer}, {}, {}
        )
...
def check_buffer_type(window, data, value):
    ...
    if (
        channel_type == "all"
        or weechat.buffer_get_string(bufpointer, "localvar_type") == channel_type
    ):
        return value
    if channel_type == "!all":
...
if __name__ == "__main__":
    if weechat.register(
        ...
        weechat.hook_config(
            "plugins.var.python." + SCRIPT_NAME + ".*", "toggle_refresh_cb", ""
        )

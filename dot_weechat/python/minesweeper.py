...
minesweeper = {
    ...
    "end": "",
    "endmsg": {
        "win": ("lightgreen", "Congratulations!"),
        "lose": ("lightred", "You blew up!"),
    },
    "color_digits": [],
    "cursor": False,
    "cheat": False,
...
minesweeper_settings_default = {
    ...
    "color_digits": (
        "21,126,34,209,201,185,15,160,9",
    ...
    "utf8": (
        "on",
        "on",
        "use utf-8 chars to draw grid and flags (your terminal/font must support these chars)",
    ),
    "zoom": (
        "",
        "",
        "zoom for board: 0 or 1 (size of squares: 0 = 4x2, 1 = 6x3 (better), empty means automatic zoom according to size of window)",
    ),
}
minesweeper_settings = {}

# mouse keys
minesweeper_mouse_keys = {
...
def minesweeper_display_status():
    ...
    if minesweeper["end"]:
        msgend = "%s%s" % (
            weechat.color(minesweeper["endmsg"][minesweeper["end"]][0]),
...
def minesweeper_display(clear=False):
    ...
    weechat.prnt_y(
        ...
        "%s%s"
        % (
            weechat.color(minesweeper_settings["color_grid"]),
    ...
    color_mine_text = "%s,%s" % (
        minesweeper_settings["color_mine"],
        minesweeper_settings["color_mine_bg"],
    )
    for y, line in enumerate(minesweeper["board"]):
        ...
        for i, str_line in enumerate(str_lines):
            weechat.prnt_y(
                minesweeper["buffer"], 1 + (y * len(str_lines)) + i, str_line
            )
...
def minesweeper_adjust_zoom():
    ...
    if minesweeper["zoom"] < 0:
        ...
        minesweeper["zoom"] = 0
        if (
            width >= minesweeper["size"] * 6
            and height >= 1 + (minesweeper["size"] * 3) + 2
        ):
            minesweeper["zoom"] = 1
    if minesweeper["zoom"] < 0:
        minesweeper["zoom"] = 0
...
def minesweeper_set_colors():
    ...
    if len(minesweeper["color_digits"]) != 9:
        weechat.prnt(
            "",
            "%sminesweeper: invalid colors (list must have 9 colors)"
            % weechat.prefix("error"),
        )
        minesweeper["color_digits"] = minesweeper_settings_default["color_digits"][
            0
        ].split(",")
...
def minesweeper_timer_cb(data, remaining_calls):
    ...
    if (
        minesweeper["buffer"]
        and weechat.buffer_get_integer(minesweeper["buffer"], "num_displayed") > 0
    ):
        minesweeper["time"] += 1
    minesweeper_display_status()
    return weechat.WEECHAT_RC_OK
...
def minesweeper_timer_start():
    ...
    if not minesweeper["timer"]:
        minesweeper["time"] = 0
        minesweeper["timer"] = weechat.hook_timer(
            1000, 0, 0, "minesweeper_timer_cb", ""
        )
...
def minesweeper_init():
    ...
    if not minesweeper["buffer"]:
        minesweeper["buffer"] = weechat.buffer_new(
            "minesweeper",
            "minesweeper_input_buffer",
            "",
            "minesweeper_close_buffer",
            "",
        )
        if minesweeper["buffer"]:
            ...
            weechat.buffer_set(
                minesweeper["buffer"], "key_bind_meta2-A", "/minesweeper up"
            )
            weechat.buffer_set(
                minesweeper["buffer"], "key_bind_meta2-B", "/minesweeper down"
            )
            weechat.buffer_set(
                minesweeper["buffer"], "key_bind_meta2-D", "/minesweeper left"
            )
            weechat.buffer_set(
                minesweeper["buffer"], "key_bind_meta2-C", "/minesweeper right"
            )
            weechat.buffer_set(
                minesweeper["buffer"], "key_bind_meta-f", "/minesweeper flag"
            )
            weechat.buffer_set(
                minesweeper["buffer"], "key_bind_meta- ", "/minesweeper explore"
            )
            weechat.buffer_set(
                minesweeper["buffer"], "key_bind_meta-n", "/minesweeper new"
            )
            weechat.buffer_set(
                minesweeper["buffer"], "key_bind_meta-+", "/minesweeper zoom"
            )
            weechat.buffer_set(
                minesweeper["buffer"], "key_bind_meta--", "/minesweeper dezoom"
            )
            weechat.buffer_set(
                minesweeper["buffer"], "key_bind_meta-c", "/minesweeper cheat"
            )
    if minesweeper["buffer"]:
        minesweeper_adjust_zoom()
        minesweeper_new_game()
...
def minesweeper_number_around(x, y):
    ...
    if (
        x >= 0
        and x <= minesweeper["size"] - 1
        and y >= 0
        and y <= minesweeper["size"] - 1
    ):
        for diff in (
            (-1, -1),
            (0, -1),
            ...
            if (
                new_x >= 0
                and new_x <= minesweeper["size"] - 1
                and new_y >= 0
                and new_y <= minesweeper["size"] - 1
            ):
...
def minesweeper_all_squares_explored():
    ...
    return (
        explored
        == (minesweeper["size"] * minesweeper["size"])
        - minesweeper["mines"][minesweeper["size"]]
    )
...
if __name__ == "__main__" and import_ok:
    if weechat.register(
        ...
        weechat.hook_config(
            "plugins.var.python.%s.*" % SCRIPT_NAME, "minesweeper_config_cb", ""
        )
